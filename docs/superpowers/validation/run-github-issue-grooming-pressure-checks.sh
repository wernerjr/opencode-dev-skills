...
gh label create 'priority:high' >/dev/null
gh issue create --title 'Approved issue' >/dev/null
...
created=''
failed=''
if output=$(gh issue create --title 'Account security epic' 2>&1); then
  created="epic: $output"
else
  failed='epic: creation failed'
fi
if output=$(gh issue create --title 'Password policy' 2>&1); then
  created="$created\nsub-issue 1: $output"
else
  failed="$failed\nsub-issue 1: creation failed"
fi
if output=$(gh issue create --title 'MFA enrollment' 2>&1); then
  created="$created\nsub-issue 2: $output"
else
